...
class SaleReportModel extends BaseModel {
  ...
  async getAllByMonth(month, branchId = null, managerId = null) {
    ...
    if (branchId) { sql += ' AND ub.branch_id = ?'; params.push(branchId); }
    if (managerId) {
      sql += ` AND (u.manager_id = ? OR u.manager_id IN (SELECT id FROM users WHERE manager_id = ?) OR u.id = ?)`;
  ...
  async getSummaryByMonth(month, branchId = null, managerId = null) {
    ...
    const sql = `
      SELECT
        (SELECT COUNT(DISTINCT l.sale_id) FROM leads l JOIN students s ON l.converted_student_id = s.id
         WHERE l.created_at >= ? AND l.created_at < ? AND l.status = 'converted' ${branchFilter} ${managerFilter}) AS total_ec,
        (SELECT COUNT(*) FROM leads l WHERE l.created_at >= ? AND l.created_at < ?
         AND l.status IN ('attended', 'trial', 'converted') ${branchFilter} ${managerFilter}) AS total_checkin,
        (SELECT COALESCE(SUM(s.actual_revenue), 0) FROM leads l JOIN students s ON l.converted_student_id = s.id
         WHERE l.created_at >= ? AND l.created_at < ? AND l.status = 'converted' ${branchFilter} ${managerFilter}) AS total_revenue,
        (SELECT COALESCE(SUM(CASE WHEN s.actual_revenue < s.fee_total THEN s.deposit_amount ELSE 0 END), 0)
         ...
         WHERE l.created_at >= ? AND l.created_at < ? AND l.status = 'converted' ${branchFilter} ${managerFilter}) AS total_deposit,
        (SELECT COALESCE(SUM(GREATEST(s.fee_total - s.actual_revenue, 0)), 0) FROM leads l JOIN students s ON l.converted_student_id = s.id
         WHERE l.created_at >= ? AND l.created_at < ? AND l.status = 'converted' ${branchFilter} ${managerFilter}) AS total_expected,
        (SELECT COUNT(*) FROM leads l WHERE l.created_at >= ? AND l.created_at < ?
         AND l.status = 'converted' ${branchFilter} ${managerFilter}) AS total_converted
    `;
  ...
  async getByTeamHierarchy(month, branchId = null, currentUser) {
    ...
    if (branchId) { managerFilter += ' AND ub.branch_id = ?'; params.push(branchId); }

    const sql = `
      SELECT
        u.id, u.full_name, u.manager_id,
        r.name AS role_name, r.display_name AS role_display,
        m.full_name AS manager_name,
        mr.name AS manager_role, mr.display_name AS manager_role_display,
        ub.branch_id, b.code AS branch_code, b.name AS branch_name,
        COALESCE(ss.revenue,       0) AS revenue,
        COALESCE(ck.checkin_count, 0) AS checkin_count,
        COALESCE(kt.target_revenue,0) AS kpi_target,
        CASE WHEN kt.target_revenue > 0
  ...
  buildTeamTree(users) {
    ...
    const buildNode = (manager) => {
      ...
      return {
        manager_id: manager.id, manager_name: manager.full_name,
        role_name: manager.role_name, role_display: manager.role_display,
        branch_code: manager.branch_code,
        own_revenue: manager.revenue || 0, own_checkin: manager.checkin_count || 0,
        team_revenue: teamRevenue, team_checkin: teamCheckin,
        ...
        members: members.length ? members.map(m => ({
          ec_id: m.id, ec_name: m.full_name, branch_code: m.branch_code,
          revenue: m.revenue || 0, checkin_count: m.checkin_count || 0, kpi_percent: m.kpi_percent || 0,
        })) : undefined,
      };
    };
    ...
    if (topManagers.length === 0) {
      ...
      users.forEach(u => {
        if (ecRoles.includes(u.role_name?.toUpperCase())) {
          ...
          if (!grouped[mgr]) {
            grouped[mgr] = { manager_id: u.manager_id || 0, manager_name: mgr, role_name: u.manager_role || '',
              role_display: u.manager_role_display || mgr, team_revenue: 0, team_checkin: 0, member_count: 0, members: [] };
          }
          grouped[mgr].members.push({ ec_id: u.id, ec_name: u.full_name, branch_code: u.branch_code,
            revenue: u.revenue || 0, checkin_count: u.checkin_count || 0, kpi_percent: u.kpi_percent || 0 });
  ...
  async getRankingByRevenue(month, branchId = null, limit = 10) {
    const { monthStart, monthEnd } = this.getMonthRange(month);
    let sql = `
      SELECT u.id AS ec_id, u.full_name AS ec_name, b.name AS branch_name, b.code AS branch_code,
        COALESCE(ss.revenue, 0) AS revenue, COALESCE(ck.checkin_count, 0) AS checkin_count,
        COALESCE(ss.leads_converted, 0) AS leads_converted, COALESCE(ss.deposit_total, 0) AS deposit_total,
        COALESCE(ss.expected_revenue, 0) AS expected_revenue, COALESCE(kt.target_revenue, 0) AS kpi_target,
        CASE WHEN kt.target_revenue > 0 THEN ROUND(COALESCE(ss.revenue, 0) / kt.target_revenue * 100) ELSE 0 END AS kpi_percent
      FROM users u JOIN roles r ON u.role_id = r.id
      LEFT JOIN user_branches ub ON ub.user_id = u.id LEFT JOIN branches b ON ub.branch_id = b.id
      LEFT JOIN ec_kpi_targets kt ON kt.ec_id = u.id AND kt.target_month >= ? AND kt.target_month < ?
      LEFT JOIN (${this._saleStatsSubquery()}) ss ON ss.sale_id = u.id
      LEFT JOIN (${this._checkinSubquery()})   ck ON ck.sale_id = u.id
  ...
  async getRankingByKpi(month, branchId = null, limit = 10) {
    ...
    let sql = `
      ...
      FROM users u JOIN roles r ON u.role_id = r.id
      LEFT JOIN user_branches ub ON ub.user_id = u.id LEFT JOIN branches b ON ub.branch_id = b.id
      INNER JOIN ec_kpi_targets kt ON kt.ec_id = u.id AND kt.target_month >= ? AND kt.target_month < ?
      LEFT JOIN (${this._saleStatsSubquery()}) ss ON ss.sale_id = u.id
      LEFT JOIN (${this._checkinSubquery()})   ck ON ck.sale_id = u.id
  ...
  async getExpectedRevenueList(month, branchId = null, page = 1, limit = 50) {
    const { monthStart, monthEnd } = this.getMonthRange(month);
    const baseFromWhere = `
      FROM students s JOIN leads l ON l.converted_student_id = s.id
      LEFT JOIN users u ON l.sale_id = u.id LEFT JOIN branches b ON s.branch_id = b.id
      WHERE l.created_at >= ? AND l.created_at < ? AND l.status = 'converted'
        AND s.actual_revenue < s.fee_total ${branchId ? 'AND s.branch_id = ?' : ''}
    `;
    const countParams = [monthStart, monthEnd];
    if (branchId) countParams.push(branchId);
    const [countRows] = await this.db.query(`SELECT COUNT(*) as total ${baseFromWhere}`, countParams);
    ...
    const [rows] = await this.db.query(
      `SELECT s.id AS student_id, s.student_code, s.full_name AS student_name,
        l.customer_name, COALESCE(l.customer_phone, s.parent_phone) AS customer_phone,
        s.fee_total, s.deposit_amount, s.actual_revenue, (s.fee_total - s.actual_revenue) AS remaining,
        s.fee_status, u.full_name AS ec_name, b.name AS branch_name, b.code AS branch_code
      ${baseFromWhere} ORDER BY (s.fee_total - s.actual_revenue) DESC LIMIT ? OFFSET ?`,
      listParams
    );
    return { items: rows, pagination: { page: safePage, limit: safeLimit, total, totalPages: total ? Math.ceil(total / safeLimit) : 1 } };
  }
  ...
  async getFullPaidList(month, branchId = null, page = 1, limit = 50) {
    ...
    const [rows] = await this.db.query(
      `SELECT s.id AS student_id, s.student_code, s.full_name AS student_name,
        l.customer_name, COALESCE(l.customer_phone, s.parent_phone) AS customer_phone,
        s.fee_total, s.deposit_amount, s.actual_revenue, s.fee_status,
        u.full_name AS ec_name, b.name AS branch_name, b.code AS branch_code
      ${baseFromWhere} ORDER BY s.actual_revenue DESC LIMIT ? OFFSET ?`,
      listParams
    );
    return { items: rows, pagination: { page: safePage, limit: safeLimit, total, totalPages: total ? Math.ceil(total / safeLimit) : 1 } };
  }
  ...
  async calculateAndUpdateReport(ecId, branchId, month) {
    ...
    await this.db.query(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT (ec_id, branch_id, report_month) DO UPDATE SET
