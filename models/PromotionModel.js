...
class PromotionModel extends BaseModel {
  ...
  async getAllItems(category = null) {
    let sql = `SELECT * FROM promotion_items WHERE is_active = true`;
    const params = [];
  ...
  async getItemsInStock() {
    const [rows] = await this.db.query(`
      SELECT * FROM promotion_items WHERE is_active = true AND stock_quantity > 0 ORDER BY category, name
    `);
    return rows;
  }
  ...
  async getStockHistory(itemId = null) {
    ...
    `;
    const params = [];
    if (itemId) { sql += ` WHERE s.item_id = ?`; params.push(itemId); }
  ...
  async getAllScholarships() {
    const [rows] = await this.db.query(`
      SELECT s.*, sub.name as subject_name FROM promotion_scholarships s
      LEFT JOIN subjects sub ON s.subject_id = sub.id WHERE s.is_active = true ORDER BY s.name
    `);
    return rows;
  }
  ...
  async getPendingApprovals(branchId = null) {
    let sql = `
      SELECT lp.*, l.student_name, l.customer_name, l.customer_phone, l.fee_total,
        pp.name as program_name, u.full_name as ec_name, b.name as branch_name
      FROM lead_promotions lp
  ...
  async getLeadGifts(leadId) {
    const [rows] = await this.db.query(`
      ...
      WHERE lg.lead_id = ? ORDER BY lg.created_at
    `, [leadId]);
    return rows;
  }
  ...
  async markGiftDelivered(giftId, userId) {
    ...
    await this.db.query(`
      UPDATE lead_promotion_gifts SET delivery_status = 'delivered', delivered_at = NOW(), delivered_by = ? WHERE id = ?
    `, [userId, giftId]);
  ...
  async returnGift(giftId, userId) {
    ...
    await this.db.query(`UPDATE lead_promotion_gifts SET delivery_status = 'returned' WHERE id = ?`, [giftId]);
  ...
  async getLowStockItems() {
    const [rows] = await this.db.query(`
      SELECT * FROM promotion_items WHERE is_active = true AND stock_quantity <= min_stock ORDER BY stock_quantity ASC
    `);
    return rows;
  }
    ...
    try {
      ...
      await conn.query(`
        ...
        SET stock_quantity = stock_quantity - ?,
            given_quantity = COALESCE(given_quantity, 0) + ?
        WHERE id = ? AND stock_quantity >= ?
      `, [quantity, quantity, itemId, quantity]);

      if (studentId) {
        await conn.query(`
          INSERT INTO gift_logs (student_id, item_id, quantity, given_by, created_at) VALUES (?, ?, ?, ?, NOW())
