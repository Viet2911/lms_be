...
class RenewalModel extends BaseModel {
  ...
  async getStudentsForRenewal(month, branchId = null, classId = null, page = 1, limit = 20) {
    ...
    let branchFilter = branchId ? 'AND s.branch_id = ?' : '';
    let classFilter  = classId  ? 'AND cs.class_id = ?' : '';
      ...
      WHERE s.status = 'active'
        AND (
          ...
          OR EXISTS (
            ...
            WHERE sr.student_id = s.id AND sr.created_at BETWEEN ? AND ?
          )
        )
        ${branchFilter} ${classFilter}
    `;

    const baseParams = [monthStart, monthEndStr, today, monthStart, monthEndStr];
    if (branchId) baseParams.push(branchId);
    if (classId)  baseParams.push(classId);
    ...
    const statsSql = `
      ...
      FROM (
        SELECT s.id,
          CASE
            WHEN EXISTS (SELECT 1 FROM student_renewals sr WHERE sr.student_id = s.id AND sr.created_at BETWEEN ? AND ?) THEN 'renewed'
    ...
    const statsParams = [monthStart, monthEndStr, today, thirtyDaysLaterStr, ...baseParams];
    const [statsRows] = await this.db.query(statsSql, statsParams);
    const stats = statsRows[0] || { total: 0, expiring: 0, expired: 0, renewed: 0 };
    ...
    const listSql = `
      SELECT
        s.id, s.student_code, s.full_name, s.parent_phone, s.fee_end_date,
        (s.fee_end_date - CURRENT_DATE) as days_remaining,
        MIN(c.id) as class_id, MIN(c.class_name) as class_name,
        MIN(c.subject_id) as subject_id, MIN(subj.name) as subject_name,
        MIN(cm.full_name) as cm_name, MIN(p.name) as package_name,
        MIN(p.sessions_count) as total_sessions, s.remaining_sessions,
      ...
      ORDER BY
        CASE
          WHEN s.fee_end_date < CURRENT_DATE THEN 1
          WHEN s.fee_end_date <= CURRENT_DATE + INTERVAL '30 days' THEN 2
          ELSE 3
        END,
        s.fee_end_date ASC
      LIMIT ? OFFSET ?
    ...
    const listParams = [monthStart, monthEndStr, today, thirtyDaysLaterStr, ...baseParams, safeLimit, offset];
    const [rows] = await this.db.query(listSql, listParams);

    return {
      students: rows, stats,
      pagination: { page: safePage, limit: safeLimit, total: parseInt(stats.total || 0), totalPages: stats.total ? Math.ceil(stats.total / safeLimit) : 1 },
    };
  }
  ...
  async createRenewal(data) {
    ...
    if (!pkgRows.length) throw new Error('Gói học không tồn tại');
    ...
    if (promotion_id) {
      ...
      if (promoRows.length) {
        ...
        promoDiscount = promo.discount_type === 'percent'
          ? Math.round(pkgPrice * (parseFloat(promo.discount_value) || 0) / 100)
    ...
    try {
      ...
      let newFeeEndDate = (student.fee_end_date && new Date(student.fee_end_date) > new Date())
        ? new Date(student.fee_end_date)
        : new Date();
      newFeeEndDate.setMonth(newFeeEndDate.getMonth() + totalMonths);

      await conn.query(`
        UPDATE students SET
          package_id = ?, fee_end_date = ?,
          ...
          fee_status = 'active', updated_at = NOW()
        WHERE id = ?
      `, [package_id, newFeeEndDate.toISOString().slice(0, 10),
        pkgPrice, promoDiscount, actualPaid, bonusMonths, pkgSessions, student_id]);
      ...
      if (renewal_type === 'new' && new_class_id) {
        ...
        if (!existingClass.length) {
          ...
          await conn.query(
            `INSERT INTO class_students (student_id, class_id, branch_id, status, enrolled_at) VALUES (?, ?, ?, 'active', NOW())`,
      ...
      if (deposit_amount > 0) {
        ...
        await conn.query(
          `INSERT INTO revenues (student_id, ec_id, amount, type, note, created_at) VALUES (?, ?, ?, 'renewal_deposit', ?, NOW())`,
  ...
  async getRenewalHistory(studentId) {
    const [rows] = await this.db.query(`
      SELECT sr.*, p.name as package_name, p.sessions,
  ...
  async getRenewalReport(month, branchId = null) {
    ...
    if (branchId) { branchFilter = 'AND cs.branch_id = ?'; params.push(branchId); }
    ...
    const [rows] = await this.db.query(`
      ...
      LEFT JOIN class_students cs ON cs.student_id = s.id AND cs.status = 'active'
      WHERE sr.created_at BETWEEN ? AND ? ${branchFilter}
