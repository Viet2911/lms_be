...
class SessionModel extends BaseModel {
  ...
  async findAllWithRelations({ classId, teacherId, branchId, fromDate, toDate, status, page = 1, limit = 20 } = {}) {
    ...
    if (status === 'pending') sql += ' AND s.attendance_submitted = false';
    else if (status === 'completed') sql += ' AND s.attendance_submitted = true';
  ...
  async rescheduleSession(sessionId, newDate, reason = null, shiftFollowing = true) {
    ...
    try {
      ...
      const [sessions] = await conn.query('SELECT * FROM sessions WHERE id = ?', [sessionId]);
      if (!sessions.length) throw new Error('Không tìm thấy buổi học');
      ...
      if (daysDiff === 0) throw new Error('Ngày mới trùng với ngày cũ');
  ...
  async cancelSession(sessionId, reason = null) {
    await this.db.query(
      `UPDATE sessions SET status = 'cancelled', cancel_reason = ?, updated_at = NOW() WHERE id = ?`,
      [reason, sessionId]
    );
    return { success: true };
  }
