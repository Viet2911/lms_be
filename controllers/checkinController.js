...
export const getCheckins = async (req, res, next) => {
  try {
    ...
    if (branch_id) { sql += ' AND uc.branch_id = ?'; params.push(branch_id); }
    if (dateFrom) { sql += ' AND uc.checkin_date >= ?'; params.push(dateFrom); }
    if (dateTo) { sql += ' AND uc.checkin_date <= ?'; params.push(dateTo); }
...
export const checkin = async (req, res, next) => {
  try {
    ...
    res.json({
      ...
      data: { id: rows[0]?.id, checkin_time: now }
    });
  } catch (error) {
    next(error);
  }
...
export const checkout = async (req, res, next) => {
  try {
    ...
    if (!existing[0]) return res.status(400).json({ success: false, message: 'Bạn chưa checkin hôm nay' });
    if (existing[0].checkout_time) return res.status(400).json({ success: false, message: 'Bạn đã checkout lúc ' + existing[0].checkout_time });

    await pool.query('UPDATE user_checkins SET checkout_time = ? WHERE id = ?', [now, existing[0].id]);
...
export const getMyStatus = async (req, res, next) => {
  try {
    ...
    res.json({ success: true, data: rows[0] || null, today });
  } catch (error) {
    next(error);
  }
};
...
export const getSubordinateSummary = async (req, res, next) => {
  try {
    ...
    const [rows] = await pool.query(`
      ...
      GROUP BY u.id, u.full_name, u.role, u.employee_code
      ORDER BY u.full_name
    `, params);

    res.json({ success: true, data: rows, period: { from: dateFrom, to: dateTo } });
  } catch (error) {
    next(error);
  }
};
...
export const adminCheckin = async (req, res, next) => {
  try {
    ...
    if (existing[0]) {
      await pool.query(
        'UPDATE user_checkins SET checkin_time = ?, checkout_time = ?, branch_id = ?, note = ? WHERE id = ?',
        [checkin_time, checkout_time, branch_id, note, existing[0].id]
      );
    } else {
      await pool.query(
        'INSERT INTO user_checkins (user_id, checkin_date, checkin_time, checkout_time, branch_id, note) VALUES (?, ?, ?, ?, ?, ?)',
  ...
  try {
    ...
    let sql = `
      SELECT
        ...
        EXTRACT(EPOCH FROM (
          COALESCE(uc.checkout_time, uc.checkin_time) - uc.checkin_time
        )) / 3600.0 as total_hours,
        CASE
          WHEN uc.checkin_time IS NULL THEN 'absent'
          WHEN uc.checkin_time::time <= '08:30:00'::time THEN 'on_time'
          ELSE 'late'
        END as status
      FROM user_checkins uc
      JOIN users u ON uc.user_id = u.id
    ...
    if (start_date) { sql += ' AND uc.checkin_date >= ?'; params.push(start_date); }
    if (end_date) { sql += ' AND uc.checkin_date <= ?'; params.push(end_date); }
    if (branch_id) { sql += ' AND uc.branch_id = ?'; params.push(branch_id); }
    ...
    let summarySql = `
      SELECT
        ...
        COUNT(*) FILTER (WHERE uc.checkin_time::time <= '08:30:00'::time) as on_time,
        COUNT(*) FILTER (WHERE uc.checkin_time::time > '08:30:00'::time) as late,
        SUM(EXTRACT(EPOCH FROM (COALESCE(uc.checkout_time, uc.checkin_time) - uc.checkin_time)) / 3600.0) as total_hours
      FROM user_checkins uc
      JOIN users u ON uc.user_id = u.id
      LEFT JOIN roles r ON u.role_id = r.id
    ...
    if (branch_id) { summarySql += ' AND uc.branch_id = ?'; summaryParams.push(branch_id); }
    if (role) { summarySql += ' AND r.name = ?'; summaryParams.push(role); }
    const [summaryResult] = await pool.query(summarySql, summaryParams);

    let staffSql = `SELECT COUNT(*) as total FROM users u LEFT JOIN roles r ON u.role_id = r.id WHERE u.is_active = true`;
    const staffParams = [];
    if (branch_id) { staffSql += ' AND u.id IN (SELECT user_id FROM user_branches WHERE branch_id = ?)'; staffParams.push(branch_id); }
    if (role) { staffSql += ' AND r.name = ?'; staffParams.push(role); }
    const [staffResult] = await pool.query(staffSql, staffParams);

    const summary = {
      total_staff: parseInt(staffResult[0]?.total || 0),
      checked_in: parseInt(summaryResult[0]?.checked_in || 0),
      on_time: parseInt(summaryResult[0]?.on_time || 0),
      late: parseInt(summaryResult[0]?.late || 0),
      absent: Math.max(0, parseInt(staffResult[0]?.total || 0) - parseInt(summaryResult[0]?.checked_in || 0)),
